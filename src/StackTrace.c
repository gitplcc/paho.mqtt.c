...
void StackTrace_entry(const char* name, int line, enum LOG_LEVELS trace_level)
{
	...
	if (!setStack(1))
		goto exit;
	if (trace_level != -1)
		Log_stackTrace(trace_level, 9, (intptr_t)my_thread->id, my_thread->current_depth, name, line, NULL);
...
void StackTrace_exit(const char* name, int line, void* rc, enum LOG_LEVELS trace_level)
{
	...
	{
		if (rc == NULL)
			Log_stackTrace(trace_level, 10, (intptr_t)my_thread->id, my_thread->current_depth, name, line, NULL);
		else
			Log_stackTrace(trace_level, 11, (intptr_t)my_thread->id, my_thread->current_depth, name, line, (int*)rc);
	}
exit:
	Thread_unlock_mutex(stack_mutex);
}
